...
class onlineSimulationWithNetwork(object):
    ...
    def runVS2(self, args, point_cloud_generator, centerline_generator):
        ...
        joint_log_file_generated = open(os.path.join("./logs", "joint_data_generated.txt"), "w+")
        ...
        while current_centerline_index < num_points - 1:
            ...
            if current_centerline_index < len(poses):
                ...
                quatCam = p.getQuaternionFromEuler([pitch_cam, roll_cam, yaw_cam])
                ...
                transformation_matrix = self.get_transformation_matrix(R_currentCam, t)
                ...
                if frame_count % 5 == 0:
                    if depth_img2 is not None and np.any(depth_img2 > 0):
                        ...
                        temp_pcd = point_cloud_generator.save_intermediate_pointcloud(
                            depth_img2, pose_img, f"intermediate_point_cloud_{frame_count}"
                        )

                        # Step 4: Compute centerline using CenterlineGenerator
                        print(f"Computing and saving centerline for frame {frame_count}...")
                        new_centerline = centerline_generator.extract_centerline_from_pcd(temp_pcd)
                        self.new_centerline = new_centerline
                        if new_centerline is not None and len(new_centerline) > 1:

                            # Find the closest point on the new centerline to the robot's current position
                            robot_pos = t  # or use your current robot position variable
                            dists = np.linalg.norm(new_centerline - robot_pos, axis=1)
                            new_index = np.argmin(dists)

                            # Update the centerline and recalculate IK
                            self.new_centerline = new_centerline
                            poses_cl, thetas_cl, phis_cl, ds_cl = kinem.inverse_kinematics_path(self.new_centerline)
                            new_current_centerline_index = new_index  # Continue from the closest point

                            # Log Cartesian position and orientation (Euler angles)
                            position = poses_cl[new_index][:3, 3]
                            rotation_matrix = poses_cl[new_index][:3, :3]
                            ...
                            # Compute new ds_offset
                            ds_offset = max(0.0, self.last_ds - ds_cl[new_index])  # Avoid negative offset

                            # Velocity and acceleration limits
                            vscaling_factor = 1.0  # Change to 0.5 or 2.0 as needed

                            vmax_base = [1, 60, 7]
                            vmax = [v * vscaling_factor for v in vmax_base]  # [cm/s, deg/s, deg/s]
                            ...
                            # Initialize previous joint values and velocities for the generated segment
                            if not hasattr(self, "prev_gen_joints"):
                                self.prev_gen_joints = [self.last_ds, phis_cl[new_index], thetas_cl[new_index]]
                                self.prev_gen_vel = [0.0, 0.0, 0.0]

                            for idx in range(new_index, min(new_index + 5, len(ds_cl))):
                                # Compute ds offset and shift
                                shifted_ds = ds_cl[idx] + ds_offset

                                # Velocity profile for ds, phi, theta
                                target_joints = [shifted_ds, phis_cl[idx], thetas_cl[idx]]
                                desired_vel = [(target_joints[i] - self.prev_gen_joints[i]) / dt for i in range(3)]

                                # Clamp velocity and acceleration
                                limited_vel = []
                                for i in range(3):
                                    v = np.clip(desired_vel[i], -vmax[i], vmax[i])
                                    delta_v = v - self.prev_gen_vel[i]
                                    max_delta_v = amax[i] * dt
                                    v = self.prev_gen_vel[i] + np.clip(delta_v, -max_delta_v, max_delta_v)
                                    limited_vel.append(v)

                                # Update joint values using limited velocity
                                new_joints = [self.prev_gen_joints[i] + limited_vel[i] * dt for i in range(3)]
                                # Only apply velocity/acceleration limits to ds (index 0)
                                #new_joints = [
                                #    target_joints[0],  # ds with limits
                                #    target_joints[1],                               # phi without limits
                                #    target_joints[2]                                # theta without limits
                                #]

                                # Clamp ds for monotonicity
                                if new_joints[0] < self.last_ds:
                                    new_joints[0] = self.last_ds
                                self.last_ds = new_joints[0]

                                for idx in range(new_index, min(new_index + 5, len(ds_cl))):
                                    # Log the generated joint data
                                    joint_log_file_generated.write(
                                        f"Frame {frame_count} - Joint Data Generated: "
                                        f"[{new_joints[0]}, {new_joints[1]}, {new_joints[2]}]\n"
                                    )
                                # Update previous values for next iteration
                                self.prev_gen_joints = new_joints
                                self.prev_gen_vel = limited_vel

                            # Assume self.centerlineArray and new_centerline are both (N, 3) arrays
                            distances = []
                            for pt in new_centerline:
                                dists = np.linalg.norm(self.centerlineArray - pt, axis=1)
                                min_dist = np.min(dists)
                                distances.append(min_dist)

                            # distances now contains the minimum distance from each point in new_centerline to the predefined centerline
                            mean_distance = np.mean(distances)
                            max_distance = np.max(distances)
                            centerline_euclidean_distance_log_file.write(
                                f"Frame {frame_count} - Mean Euclidean Distance: {mean_distance:.6f} m\n"
                            )
                            centerline_euclidean_distance_log_file.write(
                                f"Frame {frame_count} - Max Euclidean Distance: {max_distance:.6f} m\n"
                            )

                            if len(self.centerlineArray) == len(new_centerline):
                                pointwise_distances = np.linalg.norm(self.centerlineArray - new_centerline, axis=1)
                                mean_pointwise_distance = np.mean(pointwise_distances)
                                centerline_euclidean_distance_log_file.write(
                                    f"Frame {frame_count} - Mean Pointwise Distance: {mean_pointwise_distance:.6f} m\n"
                                )
                            else:
                                print("Resample both centerlines to the same number of points for pointwise comparison.")
                            ...
                            # Draw new centerline as debug lines
                            for i in range(len(self.new_centerline) - 1):
                                line_id = p.addUserDebugLine(
                                    self.new_centerline[i], self.new_centerline[i + 1],
                                    lineColorRGB=[0, 0, 1], lifeTime=0, lineWidth=3  # Blue line
                                )
                                self.debug_line_ids.append(line_id)
                            ...
                            # --- Insert this block before computing deviation ---
                            # Apply a virtual camera offset forward along the viewing direction (Z axis)
                            camera_offset = 10.45  # Adjust based on known offset (in mm or cm as appropriate)
                            view_direction = pose_img[:3, 2]  # Z-axis direction from rotation matrix (camera forward)

                            pose_img_adjusted = np.copy(pose_img)
                            pose_img_adjusted[:3, 3] += camera_offset * view_direction / np.linalg.norm(view_direction)

                            # Log both poses for debugging
                            print(f"Frame {frame_count} - Target Position:", target_pose[:3, 3])
                            print(f"Frame {frame_count} - Adjusted Camera Position:", pose_img_adjusted[:3, 3])
                            print("Target Rotation Matrix:\n", target_pose[:3, :3])
                            print("Adjusted Camera Rotation Matrix:\n", pose_img_adjusted[:3, :3])

                            

                            

                            # Select current pose from centerline
                            poses_cl_matrix = np.array(poses_cl[new_index])  # 4x4 matrix for current frame

                            # Compute translation deviation
                            translation_diff = target_pose[:3, 3] - poses_cl_matrix[:3, 3]
                            translation_dev = np.linalg.norm(translation_diff)

                            # Compute rotation deviation
                            R_target = target_pose[:3, :3]
                            R_poses_cl = poses_cl_matrix[:3, :3]
                            R_diff = R_target @ R_poses_cl.T
                            rot_diff = RR.from_matrix(R_diff)
                            euler_diff = rot_diff.as_euler('xyz', degrees=True)
                            euler_dev = np.linalg.norm(euler_diff)

                            # Print and log
                            print(f"Frame {frame_count} - Translation deviation norm: {translation_dev:.4f}")

                            kinematic_deviation_log_file.write(
                                f"Frame {frame_count} - Translation deviation (x, y, z): {translation_diff}\n"
                            )
                            kinematic_deviation_log_file.write(
                                f"Frame {frame_count} - Absolute translation deviation (x, y, z): {np.abs(translation_diff)}\n"
                            )
                            kinematic_deviation_log_file.write(
                                f"Frame {frame_count} - Rotation Difference (Euler angles): {euler_diff} (magnitude: {euler_dev:.2f}°)\n"
                            )

                            # Store for plotting
                            translation_devs.append(translation_dev)
                            rotation_devs.append(euler_dev)
