...
from .keyBoardEvents import getDirection, getDirectionBO
...
class onlineSimulationWithNetwork(object):
    ...
    def __init__(self, dataset_dir, centerline_name, renderer=None, training=True):
        ...
        euler = p.getEulerFromQuaternion([np.sqrt(2) / 2, 0, 0, np.sqrt(2) / 2]) # Euler ables given as XYZ (pitch;roll;yaw)
    ...
    def run(self, args, net, model_dir=None, epoch=None, net_transfer=None, transform_func=None, transform_func_transfer=None, training=True):
        ...
        while 1:
            ...
            nearest_centerline_point_sim_cor_index = np.linalg.norm(self.centerlineArray - t, axis=1).argmin() # ponto mais proxima na trajetoria ao atual
            ...
            if args.human:
                ...
                while(direction.sum() == 0):
                    keys = p.getKeyboardEvents()
                    direction = getDirectionBO(keys)
